...
uint32_t RCC_GetPCLK1Value(void)
{
    ...
    uint32_t tmp, ahb_prescaler, apb1_prescaler;

    // Get system clock (SYSCLK) source
    tmp = (RCC->CFGR >> 2) & 0x3; // SWS[1:0] bits
    ...
    switch(tmp)
    {
        ...
        case 1: // HSE used as system clock
            SystemCoreClock = 8000000; // HSE = 8 MHz (depends on your crystal)
            break;
        case 2: // PLL used as system clock
            // For simplicity, assume SYSCLK = PLL output
            // If using PLL, you should compute PLL output frequency here
            SystemCoreClock = 80000000; // example: 80 MHz
            break;
        default:
    ...
    uint32_t HCLK = SystemCoreClock / ahb_prescaler;

    // Get APB1 prescaler
    tmp = (RCC->CFGR >> 10) & 0x7; // PPRE1 bits
    switch(tmp)
    {
        case 0b100: apb1_prescaler = 2; break;
        case 0b101: apb1_prescaler = 4; break;
        case 0b110: apb1_prescaler = 8; break;
        case 0b111: apb1_prescaler = 16; break;
...
void USART_Init(USART_Handle_t *pUSARTHandle)
{
		...
		pUSARTHandle->pUSARTx->BRR=0X1A4;
...
//		printf("PCLK1 = %lu Hz\n", pclk1);
		...
		// Then calculate BRR for 9600 baud:
		//uint32_t baudrate = 9600;
		//uint32_t brr = 4000000 / baudrate;
		//pUSARTHandle->pUSARTx->BRR = brr;
}
